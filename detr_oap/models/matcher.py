...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_dist: float = 1):
        ...
        assert cost_class != 0 or cost_class != 0 or cost_dist != 0, "all costs cant be 0"
    ...
    def forward(self, outputs, targets):
        ...
        out_point = outputs["pred_boxes"].flatten(0, 1)  # [batch_size * num_queries, 2]

        # Also concat the target labels and boxes
        tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_point = torch.cat([v["boxes"] for v in targets])
        ...
        # Final cost matrix
        C = self.cost_dist * cost_dist + self.cost_class * cost_class
